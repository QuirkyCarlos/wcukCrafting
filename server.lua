...
RegisterServerEvent('wcukCrafting:craftStartItem')
AddEventHandler('wcukCrafting:craftStartItem',function()
	sessions[source] = {
		stoppedCraft = false,
		isCrafting = true,
		last = GetGameTimer(),
...
RegisterServerEvent('wcukCrafting:craftStopItem')
AddEventHandler('wcukCrafting:craftStopItem',function()
	sessions[source] = {
		stoppedCraft = true,
		isCrafting = false,
	}
...
RegisterServerEvent('wcukCrafting:failedCraft')
AddEventHandler('wcukCrafting:failedCraft',function(item)
...
RegisterServerEvent('wcukCrafting:craftItemDeath')
AddEventHandler('wcukCrafting:craftItemDeath',function(queueClient)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if sessions[source] then
		if sessions[source].stoppedCraft then
			...
			TriggerClientEvent('wcukNotify:Alert', source, "CRAFTING", "You died, all crafting items were given back", 5000, 'info')
		...
		TriggerClientEvent('wcukNotify:Alert', source, "CRAFTING", "No session!", 5000, 'error')
	end
			
end)

RegisterServerEvent('wcukCrafting:craftItemFinished')
AddEventHandler('wcukCrafting:craftItemFinished', function(item, crafts, itemName, isItem)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if sessions[source] then
		...
		if sessions[source].last+500 >= timeToCraft then
			...
			TriggerClientEvent('wcukNotify:Alert', source, "CRAFTING", "Anti-cheat protection!", 5000, 'error')
		end
	else
		if Webhook ~= '' then
			local identifierlist = ExtractIdentifiers(xPlayer.source)
...
ESX.RegisterServerCallback("wcukCrafting:inv2", function(source, cb, item)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("wcukCrafting:itemNames", function(source, cb)
	local itemNames = {}
...
ESX.RegisterServerCallback("wcukCrafting:CanCraftItem", function(source, cb, itemID, recipe, itemName, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
	local canCraft = true
	...
	if canCraft then
		if xPlayer.canCarryItem(itemID, amount) then
			...
			cb(true)
			TriggerClientEvent('wcukNotify:Alert', source, "CRAFTING", itemName[itemID].." added to the crafting queue", 5000, 'success')
			if Webhook ~= '' then
				local identifierlist = ExtractIdentifiers(xPlayer.source)
				local data = {
					playerid = xPlayer.source,
			...
			cb(false)
			TriggerClientEvent('wcukNotify:Alert', source, "CRAFTING", "You can't carry "..itemName[itemID], 5000, 'error')
		end
	else
		cb(false)
		TriggerClientEvent('wcukNotify:Alert', source, "CRAFTING", "You can't craft "..itemName[itemID], 5000, 'error')
	end
end)
