...
Citizen.CreateThread(function()
	...
	while true do
		...
		for k,v in pairs(Config.Crafting) do
			...
			if distance < v.radius + 2 then
				...
				if distance < v.radius then
					inZone = true
					if not Config.UsewcukTextUI and not isCraftOpen then
					...
					if IsControlJustReleased(0, Config.Key) then
						if GetVehiclePedIsUsing(PlayerPedId()) == 0 then
							...
							if not isCraftOpen then
								...
								ESX.TriggerServerCallback("wcukCrafting:itemNames", function(itemNames)
							...
							exports['wcukNotify']:Alert("CRAFTING", "You can't craft inside a vehicle", 5000, 'error')
						end
					end
				end
		...
		if inZone and not hasEntered then
			if Config.UsewcukTextUI then
				exports['wcukTextUI']:Open('[E] To open crafting table', 'darkblue', 'left') 
			end
			hasEntered = true
...
RegisterNUICallback('action', function(data, cb)
		...
		for k,v in pairs(data.crafts) do
			if data.item == v.item then
				for k2,v2 in pairs(v.recipe) do
					loop = loop + 1
					ESX.TriggerServerCallback("wcukCrafting:inv2", function(item)
						local key = item.name
	...
	elseif data.action == 'craft-button' then
		...
		ESX.TriggerServerCallback("wcukCrafting:itemNames", function(itemNames)
			ESX.TriggerServerCallback("wcukCrafting:CanCraftItem", function(canCraft)
				if canCraft then
					...
					if queue[1] == item then
						...
						while queue[1] ~= nil do
							...
							if not crafting then
								crafting = true
								SE('wcukCrafting:craftStartItem')
								local invItems = {}
								local loop = 0
								local added = 0
								...
								if IsEntityDead(PlayerPedId()) then
									...
									SE('wcukCrafting:craftItemDeath', queue)
								...
								if randomNumber <= queue[1].success then
									SE('wcukCrafting:craftItemFinished', queue[1].item, queue[1].crafts, itemNames[queue[1].item], queue[1].isItem)
									SendNUIMessage({
									...
									SE('wcukCrafting:failedCraft', itemNames[queue[1].item])
								end
